...
public class ConPERSONA {
    ...
    public boolean eliminarPersona(Persona person) throws SQLException {
        ...
        try (
            PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, person.getTipoDoc());
            pstmt.setString(2, person.getNroDocumento());
            ...
            //System.out.println(""+ex);
            System.out.println("Fallo en eliminar  bd: "+person);
...
public boolean insertarPersona(Persona person) throws SQLException {
        ...
        try (
            ...
            pstmt.setString(1, person.getTipoDoc());
            pstmt.setString(2, person.getNroDocumento());
            pstmt.setString(3, person.getNombres());
            pstmt.setString(4, person.getApellidos());
            pstmt.setString(5, person.getEmailPersonal());
            pstmt.setString(6, person.getEmailInstitucional());
            
            registro= pstmt.executeUpdate();
            
            if (registro>0) {
                System.out.println("insert exitoso  "+ person.getNroDocumento());
                respuesta=true;
            }
